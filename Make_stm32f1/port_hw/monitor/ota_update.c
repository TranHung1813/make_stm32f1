...
{
    bool retval = true;
    DEBUG_WARN("Firmware size %u bytes, download to addr 0x%08X\r\n", expected_size, OTA_UPDATE_DOWNLOAD_IMAGE_START_ADDR);
...
//        data += OTA_UPDATE_DEFAULT_HEADER_SIZE;
    ...
    while (length)
    {
        ...
        if (m_ota_remain.size == FLASH_IF_SECTOR_SIZE)
        {
            ...
            err = flash_if_erase(OTA_UPDATE_DOWNLOAD_IMAGE_START_ADDR + m_current_write_size, 
                                FLASH_IF_SECTOR_SIZE);
#endif
...
#if OTA_ERASE_ALL_FLASH_BEFORE_WRITE == 0
        flash_if_erase(OTA_UPDATE_DOWNLOAD_IMAGE_START_ADDR + m_current_write_size, 
                        FLASH_IF_SECTOR_SIZE);
#endif
...
OTA_FUNC_PLACE(void ota_update_write_header(uint32_t addr, ota_information_t *header))
{
    ...
    if (flash_if_copy(addr, (uint32_t*)&m_ota_remain.data[0], (sizeof(ota_information_t) + 3)/4) 
        != FLASH_IF_OK)
    {
        DEBUG_ERROR("Write data to flash error\r\n");
    }
...
void ota_update_enter_bootloader()
{
...
//						m_current_write_size + m_ota_remain.size);
            ...
            {
                DEBUG_ERROR("Commit flash error\r\n");
                return false;
            }
            m_ota_remain.size = 0;
        }
...
bool ota_update_verify_checksum(uint32_t begin_addr, uint32_t length)
{
    ...
    DEBUG_ERROR("Expected 0x%08X, calculated 0x%08X\r\n", expected_crc, crc);
    return false;
#endif
}
