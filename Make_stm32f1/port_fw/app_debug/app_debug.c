...
static uint8_t m_isr_buffer_size[DEBUG_ISR_RING_BUFFER_SIZE];
#endif
...
void app_debug_init(app_debug_get_timestamp_ms_cb_t get_ms, app_debug_lock_cb_t lock_cb)
{
    ...
    m_lock_cb = lock_cb;
#if DEBUG_ISR_ENABLE
    lwrb_init(&m_ringbuffer_debug_isr, &m_isr_buffer_size, DEBUG_ISR_RING_BUFFER_SIZE);
#endif
}
...
{
    if (m_debug_level == DEBUG_LEVEL_NOTHING || level < m_debug_level)
    {
        return;
    }
    if (m_lock_cb)
