...
static void invalid_crc_frame_received(min_context_t *self);
...
static void invalid_crc_frame_received(min_context_t *self)
{
    if (self->callback && self->callback->invalid_crc_callback)
    {
        ...
        self->callback->invalid_crc_callback(self, &msg);
    }
}
...
static void rx_byte(min_context_t *self, uint8_t byte)
{
    ...
    case RECEIVING_CHECKSUM_0:
        ...
        {
            invalid_crc_frame_received(self);
