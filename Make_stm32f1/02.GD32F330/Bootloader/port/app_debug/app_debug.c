...
static uint8_t m_level = APP_DEBUG_LEVEL;
...
static uint8_t m_isr_buffer[APP_DEBUG_ISR_RINGBUFFER_SIZE];
...
void app_debug_init(app_debug_get_timestamp_ms_cb_t get_ms, app_debug_lock_cb_t lock_cb)
{
    ...
    m_lock_cb = lock_cb;
#if APP_DEBUG_ISR_ENABLE
    lwrb_init(&m_ringbuffer_debug_isr, m_isr_buffer, APP_DEBUG_ISR_RINGBUFFER_SIZE);
#endif
}
...
void app_debug_print_raw(const char *fmt, ...)
{
    if (m_level == APP_DEBUG_LEVEL_NOTHING)
    {
        return;
    }
    if (m_lock_cb)
...
{
    m_level = APP_DEBUG_LEVEL_NOTHING;
}
